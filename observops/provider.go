...
func Register(name string, factory ProviderFactory) {
	...
	if factory == nil {
		panic("observops: Register factory is nil")
	}
	if _, dup := providers[name]; dup {
		panic("observops: Register called twice for provider " + name)
	}
...
func Open(name string, opts ...ClientOption) (Provider, error) {
	...
	if !ok {
		return nil, fmt.Errorf("observops: unknown provider %q (forgotten import?)", name)
	}
	return factory(opts...)
}
