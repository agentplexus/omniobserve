...
type WorkflowQuery struct {
	...
	withTasks    *AgentTaskQuery
	withHandoffs *AgentHandoffQuery
...
func (wq *WorkflowQuery) QueryTasks() *AgentTaskQuery {
	query := (&AgentTaskClient{config: wq.config}).Query()
...
func (wq *WorkflowQuery) QueryHandoffs() *AgentHandoffQuery {
	query := (&AgentHandoffClient{config: wq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := wq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (wq *WorkflowQuery) QueryEvents() *AgentEventQuery {
	query := (&AgentEventClient{config: wq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := wq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (wq *WorkflowQuery) Clone() *WorkflowQuery {
	...
	return &WorkflowQuery{
		...
		withHandoffs: wq.withHandoffs.Clone(),
...
// WithHandoffs tells the query-builder to eager-load the nodes that are connected to
// the "handoffs" edge. The optional arguments are used to configure the query builder of the edge.
func (wq *WorkflowQuery) WithHandoffs(opts ...func(*AgentHandoffQuery)) *WorkflowQuery {
	query := (&AgentHandoffClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	wq.withHandoffs = query
	return wq
}
...
func (wq *WorkflowQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workflow, error) {
	...
	if query := wq.withTasks; query != nil {
		if err := wq.loadTasks(ctx, query, nodes,
			func(n *Workflow) { n.Edges.Tasks = []*AgentTask{} },
			func(n *Workflow, e *AgentTask) { n.Edges.Tasks = append(n.Edges.Tasks, e) }); err != nil {
			return nil, err
		}
	}
	if query := wq.withHandoffs; query != nil {
		if err := wq.loadHandoffs(ctx, query, nodes,
			func(n *Workflow) { n.Edges.Handoffs = []*AgentHandoff{} },
			func(n *Workflow, e *AgentHandoff) { n.Edges.Handoffs = append(n.Edges.Handoffs, e) }); err != nil {
			return nil, err
		}
	}
	if query := wq.withEvents; query != nil {
		if err := wq.loadEvents(ctx, query, nodes,
			func(n *Workflow) { n.Edges.Events = []*AgentEvent{} },
			func(n *Workflow, e *AgentEvent) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (wq *WorkflowQuery) loadTasks(ctx context.Context, query *AgentTaskQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *AgentTask)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(agenttask.FieldWorkflowID)
	}
...
func (wq *WorkflowQuery) loadHandoffs(ctx context.Context, query *AgentHandoffQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *AgentHandoff)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Workflow)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(agenthandoff.FieldWorkflowID)
	}
	query.Where(predicate.AgentHandoff(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workflow.HandoffsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (wq *WorkflowQuery) loadEvents(ctx context.Context, query *AgentEventQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *AgentEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Workflow)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(agentevent.FieldWorkflowID)
	}
	query.Where(predicate.AgentEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workflow.EventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
